	...
	*/
	flush() {
		...
		if (this.pendingUpdates.length > 0) {
			const update = this.pendingUpdates.length === 1 ? this.pendingUpdates[0] : mergeUpdates(this.pendingUpdates);
			this.pendingUpdates = [];
			this.pendingUpdateBytes = 0;
			this.broadcastUpdate(update);
		}
...
var ClientConnection = class {
	...
	*/
	constructor(websocket, request, documentProvider, hooks, opts, defaultContext = {}) {
		...
		this.check = () => {
			...
			if (Date.now() - referenceTime > this.timeout) this.terminate(ConnectionTimeout);
		};
	...
	*/
	terminate(event) {
		...
		this.close(event);
		if (this.websocket.readyState !== WsReadyStates.Closing && this.websocket.readyState !== WsReadyStates.Closed) this.websocket.close(event?.code, event?.reason);
		clearInterval(this.pingInterval);
	}
	getQueuedBytes() {
		let total = 0;
